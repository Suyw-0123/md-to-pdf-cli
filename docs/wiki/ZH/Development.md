...
- **Docker**（`.github/workflows/docker.yml`）：觸發條件同 Publish。建置 `Dockerfile` 並推到
  `ghcr.io/suyw-0123/md-to-pdf-cli`——明確寫死並轉小寫（ghcr.io 要求小寫），而非用
  `${{ github.repository }}`（會帶 owner 的原始大小寫）；repo、PyPI 套件、image 三者同名
  `md-to-pdf-cli`，讓 `docker pull` 與 `pip install` 對得上。tag 由
...
PyPI trusted-publisher 設定必須與**目前的** repo 一致：owner `Suyw-0123`、repo
`md-to-pdf-cli`、workflow `publish.yml`、environment 留空。（repo 已從
`markdown-to-pdf-cli` 更名；舊網址會轉址，但 OIDC 檢查的是現用名稱，所以 trusted-publisher
這條必須填 `md-to-pdf-cli`。）
