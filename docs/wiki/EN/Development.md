...
- **Docker** (`.github/workflows/docker.yml`): same triggers as Publish. Builds
  the `Dockerfile` and pushes to `ghcr.io/suyw-0123/md-to-pdf-cli` — pinned
  explicitly (and lowercased) rather than `${{ github.repository }}`; repo, PyPI
  package, and image all share the name `md-to-pdf-cli`, so `docker pull` matches
  `pip install`. Tags come from `docker/metadata-action`
...
PyPI trusted-publisher config must match the **current** repo: owner `Suyw-0123`,
repo `md-to-pdf-cli`, workflow `publish.yml`, environment blank. (The repo was
renamed from `markdown-to-pdf-cli`; old URLs redirect, but OIDC checks the live
name, so the trusted-publisher entry must use `md-to-pdf-cli`.)
